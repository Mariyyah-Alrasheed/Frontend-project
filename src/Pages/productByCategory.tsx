...
export function ProductByCategory() {
  ...
  return (
    <>
      ...
      <div className="App">
        {categories
          ?.filter((cat) => cat.id === params.categoryId && <h3> {cat.name}</h3>)
          .map((cat) => (
            <>
              <h1 className="text-xl mt-44 md:text-3xl lg:text-5xl font-extrabold font-serif  uppercase mb-16 p-5 text-[#FFDAB9]">
                {cat.name}
              </h1>
              <div
                className=" m-16 flex flex-wrap w-full gap-2 mx-auto items-center justify-center"
                key={cat.id}
                id="chocolate"
              >
